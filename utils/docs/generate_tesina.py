...
def build_sommario(doc: Document) -> None:
    ...
    entries = [
        ...
        "5. Il database relazionale (scelta del modello relazionale)",
...
def add_tech_section(
    doc: Document,
    title: str,
    cos_e: str,
    funzionamento: str,
    perche_smartfare: str,
    ...
    if funzionamento:
        p = doc.add_paragraph()
        r = p.add_run("Come funziona: ")
        r.bold = True
        p.add_run(funzionamento)
        p.paragraph_format.space_after = Pt(6)
    p = doc.add_paragraph()
    r = p.add_run("Perché l'abbiamo scelta in SmartFare: ")
    r.bold = True
    p.add_run(perche_smartfare)
    p.paragraph_format.space_after = Pt(6)
    if alternative:
        p2 = doc.add_paragraph()
        r2 = p2.add_run("Alternative considerate: ")
        r2.bold = True
        p2.add_run(alternative)
...
def section_2(doc: Document) -> None:
    ...
    add_body(
        doc,
        "La scelta nasce dall'interesse per il turismo digitale e per l'intelligenza artificiale generativa "
        "applicata a un problema concreto: organizzare un viaggio in Italia usando dati reali (località, "
        "attività, hotel, coordinate) e non solo testo libero prodotto da un chatbot generico.",
    )
    add_body(doc, "Le motivazioni principali sono le seguenti:")
    add_bullet(
        doc,
        "Problema reale: pianificare un viaggio significa incrociare date, luoghi, preferenze personali "
        "e offerte locali; un'applicazione che struttura queste informazioni riduce il carico cognitivo dell'utente.",
    )
    add_bullet(
        doc,
        "Competenze del percorso di studi: il progetto integra programmazione web (frontend e backend), "
        "database relazionale, sicurezza (JWT, rate limiting, validazione input), API esterne e progettazione UX.",
    )
    add_bullet(
        doc,
        "Valore didattico per l'esame: consegnare un prodotto full-stack completo, dall'interfaccia utente "
        "alla persistenza su database, passando per l'integrazione dell'IA e le funzioni social (Discover, follow).",
    )
    add_bullet(
        doc,
        "Differenziazione tecnica: l'IA di SmartFare opera su un workspace costruito dal database "
        "(attività e hotel della località scelta), così il risultato è mappabile, modificabile e pubblicabile "
        "nella community, con meno rischio di «allucinazioni» rispetto a un assistente scollegato dai dati.",
    )
    add_body(
        doc,
        "Nota sull'evoluzione del progetto: nelle fasi iniziali il dominio del backend era orientato al "
        "confronto di biglietti ferroviari; il modello dati è stato successivamente riallineato verso itinerari "
        "turistici (POI, tappe giornaliere, visibilità pubblica/privata), mantenendo l'architettura a tre livelli "
        "(Angular, Express, PostgreSQL tramite Supabase).",
    )
...
def section_3(doc: Document) -> None:
    ...
    add_body(
        doc,
        "In questa sezione, come nell'esempio di tesina «Blocco C», per ogni tecnologia indichiamo: "
        "che cos'è, come funziona in generale e perché è stata adottata nel progetto SmartFare. "
        "Dove utile, segnaliamo anche le alternative valutate e il motivo per cui non sono state scelte.",
    )

    add_tech_section(
        doc,
        "3.1 TypeScript",
        "TypeScript è un superset di JavaScript sviluppato da Microsoft. Aggiunge tipi statici "
        "(interfacce, enum, generics) verificati in fase di compilazione; il codice viene poi transpilato "
        "in JavaScript eseguibile da browser e da Node.js.",
        "Il compilatore segnala errori prima dell'esecuzione (campo mancante, tipo errato). "
        "Gli IDE offrono autocompletamento e refactoring più sicuri su codebase grandi.",
        "In SmartFare sia il frontend (Angular 21) sia il backend (Express 5) sono scritti in TypeScript. "
        "Modelli come Itinerary, ItineraryItem, UserProfile e le risposte delle API AI hanno struttura complessa: "
        "i tipi evitano errori frequenti in JavaScript puro e rendono più chiara la collaborazione tra moduli. "
        "Sul backend gli schema Zod si integrano naturalmente con un progetto già tipizzato.",
        "Avremmo potuto usare JavaScript senza tipi, ma per un progetto d'esame full-stack la manutenibilità "
        "e la possibilità di spiegare il contratto dati alla commissione sono decisive.",
    )
    add_tech_section(
        doc,
        "3.2 Angular — framework frontend",
        "Angular è un framework per applicazioni web «single page» (SPA) mantenuto da Google. "
        "Organizza l'interfaccia in componenti, servizi iniettabili, moduli di routing e form reattivi.",
        "Il router carica i componenti senza ricaricare l'intera pagina HTML. I servizi (providedIn: 'root') "
        "sono singleton condivisi (es. AuthService, ItineraryService). I guard bloccano route non autorizzate; "
        "gli interceptor HTTP modificano ogni richiesta (JWT). Angular 21 supporta componenti standalone e signal "
        "per stato reattivo nelle schermate più dinamiche (Discover, builder).",
        "SmartFare ha molte aree funzionali distinte: home, discover, planner, builder, voyager, mappa interattiva, "
        "profilo e autenticazione. Angular offre una struttura uniforme e strumenti già pronti per routing lazy "
        "(loadComponent in app.routes.ts), form complessi e integrazione mappe (Leaflet). "
        "Per un elaborato scolastico è vantaggioso presentare un'architettura modulare riconoscibile dalla commissione.",
        "React è un'alternativa valida e molto diffusa, ma richiede più scelte manuali (router, gestione stato, "
        "struttura cartelle). Abbiamo preferito Angular per coerenza dello stack e completezza «out of the box».",
    )
    add_tech_section(
        doc,
        "3.3 Node.js ed Express — backend",
        "Node.js è un runtime che esegue JavaScript/TypeScript fuori dal browser, basato su eventi e I/O non bloccante. "
        "Express è un framework minimale per costruire server HTTP e API REST.",
        "Le richieste attraversano una catena di middleware (sicurezza, parsing JSON, autenticazione, router). "
        "Ogni route associa un verbo HTTP (GET, POST, PUT, DELETE) a una funzione controller. "
        "Express 5, usato in SmartFare, espone moduli in src/routes/ (itineraries, ai, auth, chat, profile…).",
        "Usare Node sul server permette lo stesso linguaggio del frontend: meno cambio di contesto, tipi condivisi "
        "e un unico ecosistema npm (Prisma, Zod, jsonwebtoken, SDK Gemini). Le API REST servono la SPA Angular "
        "e applicano regole di business (salvataggio itinerari, integrazione IA, upload immagini) lato server, "
        "dove i segreti (JWT_SECRET, GEMINI_API_KEY) restano al sicuro.",
        "Alternative come Java Spring o PHP Laravel sono adatte a contesti enterprise, ma avrebbero allungato "
        "i tempi di sviluppo per un team che ha già uniformato tutto su TypeScript. "
        "Su Express abbiamo aggiunto helmet (header sicurezza), rate limiting (50 richieste/15 min globali, "
        "20/min sugli endpoint AI) e CORS limitato al frontend autorizzato.",
    )
    add_tech_section(
        doc,
        "3.4 PostgreSQL, Prisma ORM e Supabase",
        "PostgreSQL è un database management system relazionale open source, conforme agli standard SQL, "
        "con transazioni ACID, vincoli di integrità e supporto a indici e relazioni. "
        "Prisma è un ORM moderno: lo schema si definisce in schema.prisma e il client TypeScript generato "
        "esegue query type-safe. Supabase ospita PostgreSQL gestito in cloud (backup, SSL, connection string).",
        "Le tabelle sono collegate da chiavi esterne (FK). Le query con JOIN recuperano dati correlati "
        "(itinerario con tappe, attività e autore) in modo efficiente. Prisma traduce le operazioni in SQL "
        "e gestisce migrazioni/schema push in sviluppo.",
        "SmartFare modella utenti, profili, preferenze, località, attività, hotel, itinerari, tappe, chat, follow "
        "e preferiti: un grafo di entità strettamente collegato. Un database relazionale garantisce che non esistano "
        "tappe senza itinerario, follow duplicati o incoerenze tra visibilità e proprietario. "
        "Discover, il builder e le API AI eseguono query articolate su più tabelle: il modello relazionale è naturale.",
        "Un database documentale (MongoDB) sarebbe flessibile per JSON variabili, ma complicherebbe vincoli e report "
        "social senza duplicare dati. SQLite è ottimo in locale, meno adatto a più utenti concorrenti in produzione. "
        "Salvare tutto su file JSON o Excel non garantirebbe concorrenza, sicurezza e integrità richieste da un'app reale.",
    )
    add_tech_section(
        doc,
        "3.5 Autenticazione: JWT, bcrypt e sessioni server-side",
        "JWT (JSON Web Token) è uno standard per token firmati che trasportano payload (es. userId, email). "
        "bcrypt è una funzione di hashing lenta per password, resistente a attacchi brute force. "
        "Le sessioni server-side memorizzano sul database quali token sono ancora validi.",
        "Alla login il server verifica credenziali, crea un record AuthSession e restituisce un JWT che include sessionId. "
        "Il client Angular invia Authorization: Bearer <token> tramite auth.interceptor. "
        "Il middleware authenticateJWT verifica firma, scadenza e che la sessione non sia revocata (logout da impostazioni).",
        "La SPA non usa sessioni cookie classiche come un sito PHP: il JWT in memoria/localStorage è lo standard per API REST. "
        "Abbinare JWT a AuthSession permette di invalidare l'accesso dal server senza attendere solo la scadenza del token. "
        "Le password locali non sono mai salvate in chiaro: solo passwordHash con bcrypt. "
        "OAuth Google/GitHub riduce attrito in registrazione; il backend verifica il token social e crea o collega l'utente.",
        "Un solo JWT senza sessione server sarebbe più semplice ma meno sicuro in caso di furto token; "
        "sessioni solo cookie richiederebbero configurazione CORS/same-site più delicata con frontend su dominio diverso.",
    )
    add_tech_section(
        doc,
        "3.6 Zod — validazione degli input",
        "Zod è una libreria TypeScript-first per definire schemi di validazione e parsare input runtime "
        "(body JSON, query string) con messaggi di errore strutturati.",
        "Ogni schema descrive campi obbligatori, tipi, formati (email, date) e relazioni. "
        "parse() o safeParse() restituiscono dati tipizzati o un errore da mappare in risposta HTTP 400.",
        "Tutti gli endpoint critici di SmartFare (registrazione, login, generazione itinerario AI, chat builder, "
        "salvataggio tappe) validano l'input prima del controller. Così non arrivano stringhe vuote, ID mancanti "
        "o JSON malformati nel database o nei prompt Gemini. Gli schema vivono in src/schemas/ e documentano "
        "il contratto API anche per chi legge la tesina.",
        "Validare «a mano» con if sparsi sarebbe error-prone; Zod centralizza le regole e allinea validazione "
        "runtime al tipo di dati atteso, complementare ai tipi TypeScript (che non esistono a runtime).",
    )
    add_tech_section(
        doc,
        "3.7 Google Gemini — intelligenza artificiale generativa",
        "I modelli linguistici di grandi dimensioni (LLM) predicono testo plausibile da istruzioni in linguaggio naturale. "
        ...
        "riducendo luoghi inventati rispetto a un chatbot generico.",
        "ChatGPT o altri LLM sono alternative equivalenti in principio; la scelta pratica dipende da quota API, "
        "policy scolastiche e integrazione già implementata. Limite dichiarato: costo per richiesta e dipendenza "
        "dalla connessione; l'IA non sostituisce il DB ma lo arricchisce l'esperienza utente.",
    )
    add_tech_section(
        doc,
        "3.8 Leaflet e OpenStreetMap",
        "Leaflet è una libreria JavaScript open source per mappe interattive nel browser. "
        "OpenStreetMap fornisce tile cartografiche libere (uso con attribuzione).",
        "La mappa si inizializza su un div HTML; si aggiungono tile layer, marker (L.marker), polilinee per percorsi "
        "e plugin come markercluster per grandi quantità di POI. invalidateSize() ridisegna dopo resize del contenitore.",
        "SmartFare visualizza tappe itinerario nel builder, percorsi in Discover e tutta Italia in interactive-map. "
        "Le coordinate provengono da PostgreSQL (Location, Activity, Accommodation). "
        "Leaflet è leggero, gratuito e adatto a progetto scolastico; Google Maps API richiederebbe billing e chiavi "
        "con vincoli di utilizzo. OSM è coerente con dati aperti e script di arricchimento POI (Overpass) usati in utils/.",
        "Mapbox o Google Maps offrono tile più «pulite» ma con costi; per dimostrare competenze GIS web Leaflet + OSM è sufficiente.",
    )
    add_tech_section(
        doc,
        "3.9 Altre integrazioni (Cloudinary, email, OAuth, sicurezza HTTP)",
        "Cloudinary è un servizio cloud per storage e trasformazione immagini. Nodemailer invia email SMTP. "
        "OAuth 2.0 permette login con provider esterni (Google, GitHub). Helmet imposta header HTTP di sicurezza; "
        "express-rate-limit limita richieste per IP.",
        "Cloudinary riceve upload da multer sul backend e restituisce URL per avatar e copertine: il database "
        "memorizza solo stringhe URL, non file binari pesanti. Nodemailer gestisce verifica email e reset password "
        "con token temporanei. angularx-social-login sul frontend avvia OAuth; il backend verifica il token. "
        "Rate limit protegge endpoint costosi (Gemini) da abuso; helmet e CORS riducono superficie d'attacco in produzione.",
        "Salvare immagini nel DB come BLOB appesantirebbe backup e query. Email e OAuth sono standard user-facing "
        "attesi in un'app moderna. Script in utils/ (CSV, Overpass, Wikidata) popolano località e attività offline, "
        "separando manutenzione dataset dall'applicazione runtime: senza POI reali l'IA sarebbe poco utile.",
        "Servizi self-hosted equivalenti (server mail proprio, storage MinIO) sono possibili ma più complessi da "
        "configurare per un elaborato scolastico; la scelta cloud accelera il time-to-market.",
    )
...
def section_5(doc: Document) -> None:
    ...
    add_heading(doc, "5.0 Perché un database relazionale", 2)
    add_body(
        doc,
        "SmartFare non memorizza solo testo generato dall'IA: persiste utenti, preferenze, itinerari con molte tappe, "
        "riferimenti a attività e hotel reali, relazioni social (follow, preferiti) e storico chat. "
        "Questi dati sono fortemente collegati tra loro e devono rispettare regole di integrità.",
    )
    add_body(
        doc,
        "Un database relazionale (PostgreSQL) è la scelta appropriata perché:",
    )
    add_bullet(
        doc,
        "Integrità referenziale: chiavi esterne impediscono tappe «orfane», follow duplicati e itinerari senza proprietario valido.",
    )
    add_bullet(
        doc,
        "Transazioni ACID: salvataggio itinerario + decine di ItineraryItem avviene in modo coerente (tutto o niente).",
    )
    add_bullet(
        doc,
        "Query complesse: Discover unisce itinerari, utenti, località, like e preferiti; il builder carica workspace "
        "con JOIN su Activity, Accommodation e categorie.",
    )
    add_bullet(
        doc,
        "Schema stabile: il dominio viaggio (giorni, ordine tappe, visibilità PUBLIC/PRIVATE) si presta a tabelle "
        "normalizzate più che a documenti JSON liberi che cambiano forma a ogni versione.",
    )
    add_body(
        doc,
        "In alternativa, un database NoSQL (es. MongoDB) offre flessibilità su documenti variabili, ma avrebbe reso "
        "più difficile garantire coerenza tra utente, itinerario e tappe senza duplicazioni. SQLite è ottimo per "
        "prototipi locali, meno adatto a deploy cloud con utenti concorrenti. File JSON o fogli Excel non supportano "
        "concorrenza, autenticazione sicura e API REST multi-utente. Per questi motivi SmartFare adotta PostgreSQL "
        "gestito tramite Prisma e hostato su Supabase.",
    )

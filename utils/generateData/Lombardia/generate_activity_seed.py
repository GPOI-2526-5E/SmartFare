...
LOMBARDIA_PROVINCES = {"MI", "BG", "BS", "CO", "CR", "LC", "LO", "MN", "MB", "PV", "SO", "VA"}
...
CATEGORY_DEFINITIONS = [
    ...
    {"id": 12, "name": "Picchi e mountains", "description": "Cime, picchi e punti notevoli in quota.", "iconUrl": None},
...
def load_locations():
    ...
    with (BASE_DIR / "locations_rows.csv").open(encoding="utf-8", newline="") as handle:
        ...
        for row in reader:
            if row["province"] not in LOMBARDIA_PROVINCES:
                continue
...
def main():
    ...
    for index, activity in enumerate(activities, start=194974):
        activity["id"] = index

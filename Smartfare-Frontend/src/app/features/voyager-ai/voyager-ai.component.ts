...
export class VoyagerAiComponent implements OnInit, AfterViewChecked {
  ...
  constructor() {
    ...
    effect(() => {
      const showReady = this.chatService.showItineraryReadyCard();
      ...
      const state = this.chatService.plannerState() || active.metadata?.plannerState;
      if (!state?.destination || !state?.locationId) return;
